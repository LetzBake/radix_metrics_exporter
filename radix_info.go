...
func systemEpochproof(baseUrl string) {
	...
	json, readErr := ioutil.ReadAll(r.Body)
	if readErr != nil {
		log.Fatal(readErr)
	}

	result := gjson.GetBytes(json, "header.nextValidators.#.stake")
...
func nodeValidator(baseUrl string) {
	...
	totalStakes := gjson.GetBytes(json, "validator.totalStake").Float()
	stakes := gjson.GetBytes(json, "validator.stakes").Array()
